...
loess(formula = LOY_percent ~ sum_Y_exp_scaled, dat) -> m

dat$predicted <- predict(m)   # Save the predicted values
dat$residuals <- residuals(m) # Save the residual values
...
if(save == T){
data.table::fwrite(x = out, file = "a:/Dropbox/LOY_microglia_paper/GR_RESUB/MANUSCRIPT/TABLES/PRE_FILTER_MALE_CELLS_3000.csv")}
...
dat$cell_type %>% table > 10 -> k
k[k==T] %>% names() -> ct

dat[dat$cell_type%in%ct,] -> dat



dat[!(dat$tissue %in% c("mBrain tumor","tumor tissue")),] -> dat

dat[dat$residuals < 0,]$residuals <- 0
...
ggboxplot(data = dat, x = "cell_type",y = "residuals", ylab = "Adjusted LOY (%)", add = "jitter")

ggscatter(data = dat, x = "donor_organism.age" , y = "residuals", size = 0.8,
          facet.by = "cell_type") + 
  geom_smooth(method = "lm",se = F) + stat_cor()

ggscatter(data = dat[dat$CLEAN=="Non-degenerative",], x = "donor_organism.age" , y = "residuals", size = 0.8,
          facet.by = "cell_type") + 
  geom_smooth(method = "lm",se = F) + stat_cor()
...
dat %>% 
  ggboxplot(color = "OTHER", y = "residuals", x = "cell_type", add = "jitter") -> p
p$data %>% group_by(cell_type) %>% summarize(mean = mean(residuals))

lm(data = dat[dat$cell_type=="Microglia" & dat$OTHER == "Control",], 
   formula = residuals ~ median_nUMI_NORMAL + donor_organism.age) %>% summary()

hist(dat$residuals, breaks = 200)
...
filter_function(min_cells = 100, score = -2, l = l,
...
dat$LOY_percent <- dat$LOY_prop * 100 %>% as.numeric()
ggscatter(dat, x = "sum_Y_exp_scaled", y = "LOY_percent", size = 0.9, color = "cell_type", facet.by = "cell_type") -> p 
  ggpar(p, xlab = "ChrY exp sparsity score (chrY-wt populations)", legend.title = "Cell type", palette = dittoColors(), legend = "bottom",
        ylab = "Loss of Y (%)") + geom_vline(xintercept = -1.75) -> p
  p + geom_smooth(se = F) -> p
...
```{r}
ggplot(dat, aes(x = sum_Y_exp_scaled, y =LOY_percent, color = cell_type) ) +
  geom_point() + 
  geom_segment(aes(xend = sum_Y_exp_scaled, yend = predicted, color = "cell_type"), alpha = .2) +
  geom_smooth(method = "loess", se = FALSE, color = "blue", ) +  # Plot
...
filter_function(min_cells = 0, score = -1.75, l = l,type = c("Astrocyte","Neuron","OPC","Microglia",
                                                             "Oligodendrocyte","Pericyte","CAM","Fibroblast")) -> dat
...
if(save == T){
data.table::fwrite(x = out, file = "a:/Dropbox/LOY_microglia_paper/GR_RESUB/MANUSCRIPT/TABLES/POST_FILTER_MALE_CELLS_3000_filtered.csv")}
...
filter_function(min_cells = 100, score = -1.25, l = l, 
                type = c("Astrocyte","Neuron","OPC","Microglia","Oligodendrocyte","Pericyte","CAM","Monocyte","Fibroblast")) -> dat
paste0(dat$sample, "_", dat$cohort) -> dat$ID
...
if(save == T){
data.table::fwrite(out, file = "a:/Dropbox/LOY_microglia_paper/GR_RESUB/MANUSCRIPT/TABLES/summarized_cell_type_LOY.csv")}
...
# LOY % by sample
dat %>% group_by(cohort, sample, cell_type) %>% summarise(LOY = sum(LOY_cells), NORMAL = sum(NORMAL_cells)) %>%
  dplyr::mutate(total = LOY + NORMAL, prop = LOY / total) -> out
...
readr::read_csv(
  "https://raw.githubusercontent.com/michaelcvermeulen/microglia-loss-of-y/main/data/LOY_tables/LOY_prop_table_3000_1000_.txt") %>%
   as.data.table() -> l
filter_function(min_cells = 100, score = -1.25, l = l, type = l$cell_type %>% unique) -> dat
paste0(dat$sample, "_", dat$cohort) -> dat$ID
...
filter_function(min_cells = 100, score = -1.25, l = l, 
                type = c("Astrocyte","Neuron","OPC","Microglia","Oligodendrocyte")) -> dat
paste0(dat$sample, "_", dat$cohort) -> dat$ID
dat %>% dplyr::distinct(ID, .keep_all = T ) %>% dplyr::group_by(neuro_degen_diagnosis) %>% dplyr::summarise(count = dplyr::n())
...
if(save == T){
ggsave(plot = tmp,
       filename = paste0(plots,"/NEURO_LOY_BRAIN_CELL_TYPES_3000_1000_no_jitter.pdf"), dpi = "retina", units = "in", height = 5, width = 12)}
...
ggboxplot(dat[dat$cell_type %in% c("Microglia","Astrocyte","Neuron","OPC","OL"),], add = "jitter",
          x = "cell_type", font.xtickslab = c(6), size = 0.2, add.params = list(size = 0.4,shape = 21),
          xlab = "", ylab = "LOY proportion", fill = "cell_type", palette = dittoSeq::dittoColors()[c(5,1,6,7,8)],
          y = "LOY_prop") %>%
          ggpar(x.text.angle = 45, legend = "none",
                font.xtickslab = c(10,"bold"), font.y = c(10,"bold") ) + coord_cartesian(ylim = c(0,0.6)) -> tmp
...
ggsave(plot = tmp,
       ...
       dpi = "retina", units = "in", height = 4.5, width = 2.5)}
...
tmp[tmp$CLEAN %in% c("Non-degenerative"),] %>%
 ...
 if(save == T){ 
ggsave(plot = a, filename = paste0(plots,"/Age_scatter_all_neuro_3000_1000.pdf"), dpi = "retina", units = "in",
       height = 4, width = 9.5)}           
            
 if(save == T){ 
ggsave(plot = b, filename = paste0(plots,"/Age_scatter_control_3000_1000.pdf"), dpi = "retina", units = "in",
       height = 4, width = 9.5)}           
...
filter_function(l=l, min_cells = 100, score = -2, type = c("Microglia","Astrocyte","Neuron","Oligodendrocyte","OPC","Pericyte")) -> dat
...
lm(data = df, formula = LOY_prop ~ sequencing_method + sum_Y_exp_scaled + OTHER + donor_organism.age) %>% summary()

ggscatter(data = df, x = "sum_Y_exp_scaled", y = "LOY_prop")
ggboxplot(data = df, x = "sequencing_method", y = "LOY_prop")
...
filter_function(l = l, score = -1.25, min_cells = 100) -> dat
...
c("Substantia nigra",
  "Temporal cortex",
  "Occipital cortex",
  "Occipitotemporal cortex", "Putamen", "Caudate nucleus", "Midbrain", 
  "Hippocampus", "Prefrontal cortex" ,"Amygdala", "Motor cortex", "Dentate gyrus", "Cingulate gyrus")
...
filter_function(l=dd, min_cells = 100, score = -1.25, type = c("Microglia","Astrocyte","Neuron","Oligodendrocyte","OPC")) -> tmp
### 
lapply(X = tmp$cell_type %>% unique, function(x){
  
  tmp[tmp$cell_type==x,] -> df
  df$LOY_percent <- df$LOY_prop * 100
  df[df$sequencing_method %in% c("10x 3' v3","10X 3' v3"),]$sequencing_method <- "10X 3' v3"
  df[df$sequencing_method %in% c("10x 3' v2","10X 3' v2"),]$sequencing_method <- "10X 3' v2"

  lm(data = df, formula = LOY_prop ~ TOTAL_CELLS + sum_Y_exp_scaled) %>% summary() -> s
  
  s$coefficients %>% as.data.frame() -> d
  d$variable <- rownames(d)
  d$cell_type <- x
  d$formula <- "LOY_percent ~ sequencing_method + TOTAL_CELLS + sum_Y_exp_scaled"
  return(d)
  
}) -> l
data.table::rbindlist(l) -> l

l[l$variable!="(Intercept)",] -> l

reshape2::dcast(data = l, formula = cell_type ~ variable, value.var = "Pr(>|t|)")
...
lapply(c("Microglia","OPC","OL","Astrocyte","Neuron"), function(x){
  ...
  ancova_model <- aov(LOY_prop ~  sum_Y_exp_scaled + TOTAL_CELLS + donor_organism.age  + AD, data = kk)
  anova(ancova_model)
  ...
  lm(data =  kk, 
     formula = LOY_prop ~ sum_Y_exp_scaled + TOTAL_CELLS + donor_organism.age + CLEAN) -> m
  summary(m) -> s 
...
ancova_model <- aov(LOY_prop ~ AD + donor_organism.age , data = kk)
postHocs <- glht(ancova_model)
summary(postHocs)
...
filter_function(l = l, min_cells = 0, score = -2, type  = l$cell_type %>% unique()) -> dat
...
dat %>% dplyr::filter(LOY_cells >= 10) -> dat
dat[dat$expr_PAR_LOY > 0, ] -> dat
dat[dat$expr_PAR > 1, ] -> dat
  

factor(dat$cell_type, levels = c("Microglia","Astrocyte","Neuron","Oligodendrocyte","OPC","CAM","Pericyte",
                                 "T cell", "NK", "Monocyte", "Macrophage","ILC","Kupffer","cDC", "pDC",
                                 "B cell","Fibroblast","Endothelial","Epithelial")) -> dat$cell_type

dat[!is.na(dat$cell_type),] -> dat
ggstripchart(dat, x = "cell_type", y = "diff_PAR", add = "mean_sd", fill = "cell_type",
             add.params = list(color = "red"), palette = dittoColors()[c(5,1,6,7,8,2,3,4,9,10:19)],
            shape = 21, stroke = 0.8, size = 0.7) %>% 
  ggpar(ylab = "logFC PAR (NORMAL v LOY)", xlab = "Cell type", font.xtickslab = c("bold",8), legend = "none", 
...
ggsave(plot = p, filename = "a:/Dropbox/LOY_microglia_paper/GR_RESUB/MANUSCRIPT/PLOTS_FOR_REVIEWERS/plots/PAR_logFC_stripchart.pdf",
       dpi = "retina", width = 6, height = 9, units = "in"
...
###################################### 1500 ######
readr::read_csv(
  "https://raw.githubusercontent.com/michaelcvermeulen/microglia-loss-of-y/main/data/LOY_tables/all_cell_types/LOY_prop_table_1500_1000.txt") %>%
   as.data.table() -> l

filter_function(l = l, min_cells = 0, score = -2, cell_type = l$cell_type %>% unique()) -> dat
...
ggscatter(dat, y = "diff_PAR", x = "LOY_prop", color = "cell_type") %>%
  ggpar(x.text.angle = 45, font.xtickslab = c(10,"bold"), xlab = "LOY proportion", ylab = "") %>% ggpar(legend = "none") -> p
