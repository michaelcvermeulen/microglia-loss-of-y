...
l[l$TOTAL_CELLS > min_cells & l$sum_Y_exp >= sum_Y_exp, ] -> l
...
filter_function(min_cells = 0, sum_Y_ex = 0, l = l) -> dat
...
# LOY % by sample 
paste0(dat$sample,"_",dat$cohort) -> dat$ID
dat %>% group_by(ID) %>% summarise(LOY = sum(LOY_cells), NORMAL = sum(NORMAL_cells)) %>%
  dplyr::mutate(total = LOY + NORMAL, prop = LOY / total) -> out
