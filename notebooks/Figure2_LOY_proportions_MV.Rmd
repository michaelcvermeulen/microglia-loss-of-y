...
filter_function <- function(min_cells=75, sum_Y=250,l , 
                            cell_type = c("Oligodendrocyte","Microglia","Astrocyte","Neuron","OPC","Pericyte")){
## cleans up 
  
l[l$TOTAL_CELLS > min_cells & l$sum_Y_exp >= sum_Y, ] -> l
l[l$cell_type %in% cell_type,] -> dat
...
filter_function(min_cells = 0, sum_Y = 0, l = l) -> dat
...
ggscatter(dat, x = "sum_Y_exp", y = "LOY_prop", facet.by = "cell_type", size = 0.4) + geom_vline(xintercept = 250) -> p 
  ggpar(p, xlab = "MSY pct expressed in non-LOY cells/nuclei")
...
gghistogram(dat, x = "median_nUMI_NORMAL", facet.by = "cell_type")
ggscatter(dat, x = "median_nUMI_NORMAL", y = "LOY_prop", facet.by = "cell_type", size = 0.4) -> p
  ggpar(p, xlab = "Median nCount") + geom_smooth(se = F)
...
data.table::fread(paste0(output,"/LOY_prop_table_3000_1000.txt")) -> l
filter_function(min_cells = 75, sum_Y_ex = 250, l = l) -> dat
paste0(dat$sample, "_", dat$cohort) -> dat$ID
...
dat %>% group_by(ID, cell_type) %>% summarise(LOY = sum(LOY_cells), NORMAL = sum(NORMAL_cells)) %>%
  dplyr::mutate(total = LOY + NORMAL, prop = LOY / total) -> out
...
dat %>% dplyr::group_by(cell_type) %>% dplyr::summarise(LOY = sum(LOY_cells), NORMAL = sum(NORMAL_cells)) %>%
  ...
  ggbarplot(x = "cell_type", y = "LOY_prop", palette = dittoSeq::dittoColors()[c(5,8,7,1,6)], fill = "cell_type") %>%
...
ggboxplot(dat,
          x = "cell_type", font.xtickslab = c(6), size = 0.2, outlier.size= 0.6, 
...
ggsave(plot = p, filename = "a:/Dropbox/LOY_microglia_paper/GR_RESUB/MANUSCRIPT/PLOTS_FOR_REVIEWERS/plots/normPAR_violin_SMITH_AM.pdf",
       dpi = "retina", width = 10, height = 6, units = "in"
...
ifelse(o.@meta.data$LOY == "LOY", 1, 0) %>% as.factor() -> o.@meta.data$LOY_factor
lm(data = o.@meta.data, formula = LOY_factor ~ PAR_score1 + nCount_RNA + nFeature_RNA ) %>% summary()
