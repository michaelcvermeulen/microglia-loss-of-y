...
LOY_DE <- function(s_obj, cell_type, subject, top = "10%", bottom = "90%", logfc = 0.25, min.pct = 0.1){
  
  Seurat::Idents(s_obj) <- "LOY" ## if statement here 
  
  cells <- s_obj@meta.data[s_obj@meta.data$donor_organism.biomaterial_core.biomaterial_id %in% c(subject) & 
                             s_obj@meta.data$cell_ident_MV %in% c(cell_type),] %>% rownames()
  
  if(length(cells) < 1){return(NULL)}
  
  s_obj <- subset(s_obj, cells = cells)
  
  quantile(s_obj@meta.data$nCount_RNA, probs = seq(0,1,0.01)) %>% as.data.frame() -> d
  subset(s_obj, nCount_RNA < as.numeric(d[top,]) & nCount_RNA > as.numeric(d[bottom,])) -> s_obj
  print(table(s_obj@meta.data$LOY))
  
  table(s_obj@meta.data$LOY) %>% as.matrix() %>% as.data.frame() -> a
  
  if(length(a$V1)!=2){print("not enough cells!"); return(NULL)}
  if(a$V1[1]<10){print("not enough cells!"); return(NULL)}
  
  DE <- Seurat::FindMarkers(s_obj, ident.1 = "LOY", ident.2 = "NORMAL",
                            min.pct = min.pct,
                            logfc.threshold = logfc)
  
  s_obj@meta.data %>% group_by(cell_ident_MV,LOY) %>% 
    summarize(n = dplyr::n(), 
              mean_norm_Y = mean(normY), 
              mean_norm_PAR = mean(normPAR), 
              mean_Y_genes = mean(chrY_genes), 
              mean_nUMI = mean(nCount_RNA),
              mean_nGene = mean(nFeature_RNA)) %>% print()
...
enrichment <- function(DE, sig = 0.001){
  require(enrichR)
  rownames(DE[DE$p_val_adj < sig,]) -> genes
...
## output the genes that should be excluded from the LOY analysis. 
## compare the males to females for the Y genes  
